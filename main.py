...
if "messages" not in st.session_state:
    ...
    st.session_state.current_level = ""  # レベル追跡用の新しい変数を追加
    ...
    # モード「日常英会話」用のChain作成は後で行う（レベル選択後）
...
# レベルが変更された場合の処理
if st.session_state.current_level != st.session_state.englv:
    st.session_state.current_level = st.session_state.englv
    # 日常英会話用のチェーンを再作成
    if hasattr(st.session_state, 'englv'):
        conversation_template = ct.SYSTEM_TEMPLATE_BASIC_CONVERSATION.format(level=st.session_state.englv)
        st.session_state.chain_basic_conversation = ft.create_chain(conversation_template)
...
if st.session_state.start_flg:
    ...
    if st.session_state.mode == ct.MODE_3 and (st.session_state.dictation_button_flg or st.session_state.dictation_count == 0 or st.session_state.dictation_chat_message):
        if st.session_state.dictation_first_flg:
            # レベルと単語数を含む完全なプロンプトを作成してからチェーンを作成
            import random
            ...
            complete_template = ct.SYSTEM_TEMPLATE_CREATE_PROBLEM.format(
                level=level, 
                word_count=word_count
            )
            st.session_state.chain_create_problem = ft.create_chain(complete_template)
